            ...
            try:
                if session.platform == "threads":
                    ...
                    with connection.cursor() as cursor:
                        ...
                        # (n8n may store only part of the session_id)
                        if not row:
                            cursor.execute(
                                "SELECT id, script_text as script, NULL as title, "
                                "NULL as description, NULL as tags, video_url, status "
                        ...
                        # (n8n credential user_id doesn't match Django creator_id)
                        if not row:
                            cursor.execute(
                                "SELECT id, script_text as script, NULL as title, "
                                "NULL as description, NULL as tags, video_url, status "
            ...
            if platform and platform.lower() == 'threads':
                try:
                    ...
                    if tgv_row:
                        ...
                        # Create a lightweight creator_session so polling works
                        synthetic_session_id = tgv_session_id or f"threads_synth_{tgv_id}"
                        obj, created = CreatorSession.objects.get_or_create(
...
class N8NCallbackView(APIView):
    ...
    def post(self, request):
        ...
        if action == "script_ready":
            ...
            # Bridge logic for Django session
            CreatorSession.objects.filter(session_id=session_id).update(status="script_pending")
            if creator_id:
                django_session = CreatorSession.objects.filter(creator_id=creator_id).exclude(session_id=session_id).order_by('-created_at').first()
                if django_session:
        ...
        if action == "video_ready":
            ...
            if session_id and session_id.startswith("threads_"):
                ...
                with connection.cursor() as cursor:
                    ...
                    # Also update any row that already has this session_id
                    cursor.execute(
                        "UPDATE threads_generated_videos SET video_url = %s, status = 'done' WHERE session_id = %s",
                        [video_url, session_id]
                    )

                # Also persist to GeneratedVideo so SessionStatusView fallback works
